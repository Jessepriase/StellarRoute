...
async fn get_markets_returns_same_as_pairs() {
    ...
    let pairs_json: serde_json::Value = serde_json::from_slice(&pairs_body).expect("Pairs body is not valid JSON");
    ...
    let markets_json: serde_json::Value = serde_json::from_slice(&markets_body).expect("Markets body is not valid JSON");
